...
class HAPI(API):
    """Higher level API interface, including some feedback, robot state
    checking and so on."""
    ...
    def RecordingActions(self):
        ...
        def __cap_one_point():
            data = self.rtif.receive()
            cap = {
                'Tool Position': data['Actual Tool Coordinates'][:],
        ...
        while True:
            i = input(
                "Input 'Enter' to capture, 'e+Enter' to exit, m+Enter to enter edit mode >>"
            )
            if i == 'e':
                break
            elif i == '':
                ...
                while True:
                    for ind, c in enumerate(caps):
                        if ind == current_id:
                            print(
                                '->%02d : End point (%04f, %04f, %04f, %04f, %04f, %04f)'
                                % ((ind, ) + c['Joint Position']))
                        else:
                            print(
                                '  %02d : End point (%04f, %04f, %04f, %04f, %04f, %04f)'
                                % ((ind, ) + c['Joint Position']))
                    ...
                    if j == 'u':
                        ...
                        while self.is_direct_mode(
                        ) and not self.isLastMovementEnd():
                            time.sleep(0.5)
                        self.TeachMode(18000)
                    elif j == 't':
                        caps.pop(current_id)
                        ...
                        cap = __cap_one_point()
                        if current_id == 0:
                            cap['Time Step'] = caps[current_id][
                                'Time Step'] - 3.0
                        else:
                            cap['Time Step'] = (
                                caps[current_id]['Time Step'] +
                                caps[current_id - 1]['Time Step']) / 2.0
    ...
    def ReplayCapturedData(self, caps, with_time=False):
        ...
        if with_time:
            ...
            for i in range(1, len(caps)):
                self.MoveJointToRad(
                    caps[i]['Joint Position'],
                    t=(caps[i]['Time Step'] - caps[i - 1]['Time Step']))
                while self.is_direct_mode() and not self.isLastMovementEnd():
                    time.sleep(0.5)
        else:
            for cap in caps:
    ...
    def set_coordinate_origin(self, ori=None):
        """Setting the Coordinate origin point. If ori is None, will
        automatically use teach mode. operating coordinate is: +X from robot
        center to end tool, +Z toward sky. tool coordinate is: the connector of
        tool towards +X and tool face -Z.
        ...
        if len(ori) != 2:
            ...
            q, t = quat.from_vector_to_q((1, 0, 0), unio + unix, (0, 0, 1),
                                         unio + uniz, (0, 0, 0), unio)
            ori = (t, q)
    ...
    def GetCurrentEndForce(self):
        ...
        return np.asarray((f[0], f[1], f[2], t[0], t[1], t[2]),
                          dtype=np.float32)
