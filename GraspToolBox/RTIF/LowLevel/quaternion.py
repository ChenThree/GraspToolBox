...
def qrotote_v(q0, v1, trans=(0, 0, 0)):
    """rotate the vector, if want rotate the coordinate, use
    qrotate_v(qconj(q0), v1)"""
    q1 = np.zeros_like(q0)
    q1[1:] = v1
    return qmul(qmul(q0, q1), qconj(q0))[1:] + np.asarray(
...
def to_angle_axis(q0):
    ...
    return np.array([rad * 2.0, q0[1] / sin_x, q0[2] / sin_x, q0[3] / sin_x],
                    dtype=np.float32)
...
def from_matrix_to_q(mat):
    ...
    qi = math.copysign(
        math.sqrt(max(0, 1 + mat[0][0] - mat[1][1] - mat[2][2])) / 2.0,
        mat[2][1] - mat[1][2])
    qj = math.copysign(
        math.sqrt(max(0, 1 - mat[0][0] + mat[1][1] - mat[2][2])) / 2.0,
        mat[0][2] - mat[2][0])
...
def from_angle_axis(a):
    return np.asarray(
        (math.cos(0.5 * a[0]), math.sin(0.5 * a[0]) * a[1],
         math.sin(0.5 * a[0]) * a[2], math.sin(0.5 * a[0]) * a[3]),
        dtype=np.float32)
...
def from_vector_to_q(v1_c1,
    ...
    c1 = np.asarray(
        (v1_c1 - v3_c1, v2_c1 - v3_c1, np.cross(v1_c1 - v3_c1, v2_c1 - v3_c1)),
        dtype=np.float32).T
    c2 = np.asarray(
        (v1_c2 - v3_c2, v2_c2 - v3_c2, np.cross(v1_c2 - v3_c2, v2_c2 - v3_c2)),
        dtype=np.float32).T
    mat = c2.dot(c1.T).dot(np.linalg.pinv(c1.dot(c1.T)))
    return from_matrix_to_q(mat), (v1_c2 - mat.dot(v1_c1) + v2_c2 -
                                   mat.dot(v2_c1) + v3_c2 -
                                   mat.dot(v3_c1)) / 3.0


def from_vector_array_to_q(v_c1, v_c2):
    """Calculate transform quaternion and translation vector from vector pairs
    in two coordinate.
    ...
    if len(v_c1) != len(v_c2) or len(v_c1) <= 3:
        print(
            'Error! on enough vector pair or length of two array is different')
...
if __name__ == '__main__':
    ...
    T = np.asarray(((-1, 0, 0, 2), (0, 0, 1, 3), (0, 1, 0, 4)),
                   dtype=np.float32)
    ...
    q, t = from_vector_to_q(v1[:3], v1_n[:3], v2[:3], v2_n[:3], v3[:3],
                            v3_n[:3])

    print(v1_n, qrotote_v(q, v1[:3], t), v2_n, qrotote_v(q, v2[:3], t), v3_n,
          qrotote_v(q, v3[:3], t))
