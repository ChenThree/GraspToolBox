...
class API(object):
    """Low level API interface, only make factory API call in UR5 controller
    remotely."""
    ...
    def from_rad_axis_to_q(r):
        ...
        if norm_axis < 1e-5:
            ...
            return np.asarray(
                (math.cos(0.5 * norm_axis), math.sin(0.5 * norm_axis) * r[0] /
                 norm_axis, math.sin(0.5 * norm_axis) * r[1] / norm_axis,
                 math.sin(0.5 * norm_axis) * r[2] / norm_axis),
                dtype=np.float32)
    ...
    def MoveJointToRad(self, q, a=1.4, v=0.5, t=None):
        """
    ...
    def GetCurrentJointRad(self):
        ...
        return np.asarray(
            self.rtif.receive()['Actual Joint Positions'], dtype=np.float32)
    ...
    def GetTargetJointRad(self):
        ...
        if not self.__direct_control_mode:
            print('Warning: in buffered mode, Get operation will be ignored!')
        return np.asarray(
            self.rtif.receive()['Target Joint Positions'], dtype=np.float32)
    ...
    def GetCurrentEndPos(self):
        ...
        return np.asarray(
            ret[:3], dtype=np.float32), self.from_rad_axis_to_q(ret[3:])
    ...
    def GetCurrentEndForce(self):
        ...
        if not self.__direct_control_mode:
            print('Warning: in buffered mode, Get operation will be ignored!')
        return np.asarray(
            self.rtif.receive()['Generalized Tool Force'], dtype=np.float32)

    def Sleep(self, sec=1.0):
        """let robot sleep for some time (second).

        can only be used in buffered command mode
        :return: None
        """
        if self.__direct_control_mode:
            print(
                'Warning: you are not in buffered mode, command sleep ignored')
        else:
    ...
    def switch_mode(self, direct_mode):
        """switch between direct mode and buffered command mode. will clear
        buffer even if not switch.
